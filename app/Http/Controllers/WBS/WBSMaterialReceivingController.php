    ...
    public function postSaveMaterialReceiving(Request $req)
    {
        if ($req->savestate == 'ADD') {
            ...
            if (count((array)$batchdata) > 0) {
                ...
                //     if ($this->checkIfBatchExist($value->id)) {
                //         DB::connection($this->mysql)->table('tbl_wbs_material_receiving_batch')
                //             ->where('id',$value->id)
                //             ->update([
                //                 'qty' => str_replace(',','', $value->qty),
                //                 'box' => $value->box,
                //                 'box_qty' => str_replace(',','', $value->box_qty),
                //                 'lot_no' => $value->lot_no,
                //                 'supplier' => strtoupper($value->supplier),
                //                 'update_user' => Auth::user()->user_id,
                //                 'updated_at' => date('Y-m-d H:i:s')
                //             ]);
                ...
                //         if (floatval($check->qty) == floatval(str_replace(',','',$value->qty))) {
                //             DB::connection($this->mysql)->table('tbl_wbs_inventory')
                //                 ->where('mat_batch_id',$value->id)
                //                ->update([
                //                     // 'qty' => str_replace(',','',$value->qty),
                //                    'box' => $value->box,
                //                    'box_qty' => str_replace(',','',$value->qty),
                //                    'lot_no' => $value->lot_no,
                //                    'supplier' => strtoupper($value->supplier),
                //                    'update_user' => Auth::user()->user_id,
                //                    'updated_at' => date('Y-m-d H:i:s')
                ...
                //         $this->UpdateCalculateQty($mrdata->receive_no,$value->item,str_replace(',','',$value->qty));
                    } else {
                        $check = 0;
                        $printed = 0;
