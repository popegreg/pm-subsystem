...
class WBSWhsIssuanceController extends Controller
{
    ...
    public function getTotalIssuedQty(Request $req)
    {
    	try {
    		// $total_issued_qty = DB::connection($this->mysql)->table('tbl_wbs_warehouse_mat_issuance_details')
			// 					->where('request_no',$req->request_no)
			// 					->where('item',$req->item)
			// 					->select(DB::raw('SUM(request_qty) as total_request_qty')
			// 							,DB::raw('SUM(issued_qty_t) as served_qty'))
			// 					->first();
			// $old_issued_qty_t = DB::connection($this->mysql)->table('tbl_wbs_warehouse_mat_issuance_details')
			// 					->where('id',$req->id)
			// 					->select('issued_qty_t')
			// 					->first();
			$total_issued_qty = DB::connection($this->mysql)->table('tbl_wbs_warehouse_mat_issuance_details')
								->where('request_no',$req->request_no)
								->where('item',$req->item)
								->select(DB::raw('IFNULL(SUM(request_qty),0) as total_request_qty')
										,DB::raw('IFNULL(SUM(issued_qty_t),0) as served_qty'))
								->first();
			$old_issued_qty_t = DB::connection($this->mysql)->table('tbl_wbs_warehouse_mat_issuance_details')
								->where('id',$req->id)
								->select(DB::raw('IFNULL(issued_qty_t,0) as issued_qty_t'))
								->first();
    ...
    public function downloadBarcode(Request $req)
    {
		$path = storage_path().'/brcodekitting';

	    $headers = [
	    	'Content-type'=>'text/plain',
	    	'Content-Disposition'=>sprintf('attachment; filename="%s"', $req->filename)
	    ];
	    return \Response::download($path.'/'.$req->filename, $req->filename, $headers);
    }
}