$(function(){
	...
	$('#btn_force_dl').on('click',function(){
		...
		if(checkedpline == 0 && checkedpertable == 0 ){
			...
			msg('Please select atleast 1 productline and 1 table to download','failed');
			return false;
		}
		if(checkedpline == 0){
			msg('Please select atleast 1 productline','failed');
			return false;
		}
	    if(checkedpertable == 0){
			msg('Please select atleast 1 table','failed');
			return false;
		}
		$('#loading').modal('show');	
		...
		$.each(checkboxValues,function(index, value) {	
		    ...
		    // msg('Download successfully!','sucess');
	});
