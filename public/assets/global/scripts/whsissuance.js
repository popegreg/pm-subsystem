...
$( function() {
	...
	$('#tbl_issuance_body').on('click', '.btn_edit_issuance_details', function() {
		...
		// var find_item = issuance_head.indexOf($(this).attr('data-item'));
		var data_req_qty = $(this).attr('data-request_qty');
		var data_item =$(this).attr('data-item');
		var find_item = -1;
		$.each(issuance_head, function(index, val) {
		 ...
		 //    console.log(data);
			...
			if (data.status == 'success') {
				 ...
				 data_served_qty = parseFloat(data[0].served_qty);
			 	 if (find_item >= 0 ) {
					 $('#edit_served_qty').val(issuance_head[find_item].served_qty);;
				 }else{
				 	 $('#edit_served_qty').val(data_served_qty);
				 }
				 
				 console.log(find_item);
				 console.log('edit_served_qty' +  $('#edit_served_qty').val());
	...
	$('#btn_update_details').on('click', function() {
		...
		var total_issued = parseFloat($('#edit_served_qty').val()) - parseFloat($('#old_issued_qty').val());
		 ...
		 console.log('total_issued:'+ total_issued);
		...
		if (total_issued > parseFloat($('#total_request_qty').val())) {
			...
			if (find_item >= 0) {
				    issuance_items = [];
					issuance_items.item = $('#edit_item').val();
					issuance_items.served_qty = total_served_qty;
					issuance_head.splice(find_item,1,issuance_items);
			}else{
				    issuance_items = [];
					issuance_items.item = $('#edit_item').val();
					issuance_items.served_qty = total_served_qty;
					issuance_head.push(issuance_items);
			}
			console.log(issuance_head);
...
function printBRcode(arr_data) {
	$.ajax({
		...
		window.location.href = downloadBarCodeURL+"?filename="+data.filename;
	}).fail(function(xhr,textStatus,errorThrown) {
